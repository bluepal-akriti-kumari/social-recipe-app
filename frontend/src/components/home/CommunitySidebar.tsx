    ...
    >
      ...
      <Box>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {['JamieOliver', 'GordonR', 'ChefMario'].map((nick) => (
            <Box key={nick} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                <Avatar sx={{ width: 32, height: 32, fontWeight: 900, fontSize: 12 }}>{nick[0]}</Avatar>
                <Typography variant="subtitle2" sx={{ fontWeight: 900, fontSize: '0.8rem' }}>{nick} <VerifiedIcon sx={{ fontSize: 12, color: 'primary.main' }} /></Typography>
              </Box>
